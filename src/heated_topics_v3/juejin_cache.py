"""Daily single-flight caches for Juejin rank / search / article fetches.
...
BOARD_SUBDIR = "juejin/rank"
SEARCH_SUBDIR = "juejin/search"
ARTICLE_SUBDIR = "juejin/articles"
...
def _atomic_write_json(path: Path, payload: dict) -> None:
    ...
    fd, tmp_name = tempfile.mkstemp(prefix=".juejin_cache_", suffix=".json", dir=str(path.parent))
    try:
    ...
    *,
    ...
    def load_cached() -> dict | None:
        ...
        if payload.get("date") != date:
            return None
        value = {k: v for k, v in payload.items() if k not in {"schema_version", "date"}}
        if not value:
            return None
        return value
...
def get_or_fetch_board(
    ...
    **kwargs: Any,
    ...
    return get_or_fetch_board_with_record(cache_root, date, fetch_live, **kwargs)
